#!/usr/bin/env node
/**
 * Lindcott Armory — Reference Data Seed Script
 *
 * Upserts gun_history and service_weapons tables from the lindcott-website
 * data files. Safe to re-run; uses upsert (ON CONFLICT DO UPDATE).
 *
 * Usage:
 *   SUPABASE_SERVICE_KEY=your_service_role_key node seed-reference-data.js
 *
 * Requires:
 *   npm install @supabase/supabase-js   (already present in gun-app/web)
 *   Or run from the gun-app/web directory where it is already installed.
 *
 * The script resolves @supabase/supabase-js from the web package if needed.
 */
...
if (!SUPABASE_SERVICE_KEY) {
  ...
  console.error('Usage: SUPABASE_SERVICE_KEY=xxx node seed-reference-data.js');
  process.exit(1);
}

// ---------------------------------------------------------------------------
// Resolve @supabase/supabase-js — look in gun-app/web node_modules first
// ---------------------------------------------------------------------------
let createClient;
try {
  ...
  try {
    ...
    const webModules = path.resolve(__dirname, '../web/node_modules/@supabase/supabase-js');
    ({ createClient } = require(webModules));
  } catch (err) {
    console.error('ERROR: Could not find @supabase/supabase-js.');
    console.error('Run: cd gun-app/web && npm install, then retry from gun-app/supabase/.');
...
// ---------------------------------------------------------------------------
// GUN HISTORY DATA
// Source: /Users/banner-james/Desktop/lindcott-website/data/gun-history-data.js
// ---------------------------------------------------------------------------
const GUN_HISTORY = [
  {
    id: 'flintlock-musket',
...
function loadCartridges() {
  ...
  return raw.map(c => ({
    ...
    law_enforcement_use:    c.lawEnforcementUse || false,
...
async function seedTable(tableName, rows) {
  ...
  // Supabase upsert in batches to avoid URL length limits
  const BATCH_SIZE = 50;
  let inserted = 0;
...
// ---------------------------------------------------------------------------
async function main() {
  console.log('Lindcott Armory — Reference Data Seed');
  console.log('Supabase URL:', SUPABASE_URL);
  console.log('');

  try {
    await seedTable('gun_history', GUN_HISTORY);
    await seedTable('service_weapons', SERVICE_WEAPONS);
    const cartridges = loadCartridges();
    await seedTable('cartridges', cartridges);
    console.log('\nAll done. Reference data seeded successfully.');
  } catch (err) {
